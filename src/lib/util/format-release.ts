export const formatRelease = (date: string) => {
	...
	if (daysDifference < 1) {
		...
		const hourFormat = rtf1.format(
			-parseInt(hoursDifference.toFixed(0)),
			'hour',
		);
		...
		const isDateFromLastWeek =
			previousDate >= oneWeekAgo && previousDate < new Date();
		...
		const rtf3 = new Intl.DateTimeFormat('en', {
			month: 'short',
			day: 'numeric',
		});

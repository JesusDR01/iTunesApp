...
export default async function handler(
	...
	try {
		const { handle: podcastId } = req.query;
		...
		if (typeof podcastId === 'string') {
			...
			const podcastDetails: Podcasts = rawEpisodes
				.filter(episode => {
					const timeDifference =
						Date.now() - new Date(episode.releaseDate).getTime();
					return timeDifference > 0;
				})
				.sort(
					(prevEpisode, currentEpisode) =>
						new Date(currentEpisode.releaseDate).getTime() -
						new Date(prevEpisode.releaseDate).getTime(),
				)
				.map(episode => ({
					...
					duration: episode.trackTimeMillis
						? new Date(episode.trackTimeMillis).toISOString().substr(11, 8)
						: '',
			...
			res
				.status(200)
				.json({ podcastDetails, podcastTitle: rawEpisodes[0].collectionName });
		} else {
